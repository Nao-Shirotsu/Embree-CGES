...
PolygonalMesh::PolygonalMesh(const RTCDevice device, 
                             const glm::vec3& posWorld, 
                             const char* const filePath, 
                             const ColorRGBA diffuseColor, 
                             const ColorRGBA emissionColor,
                             const material::Base& material) 
    : Base(device, RTC_GEOMETRY_TYPE_TRIANGLE, emissionColor, material)
    , m_posWorld(posWorld)
    , m_diffuseColor(diffuseColor){
#ifdef _DEBUG
  assert(obj::LoadFromFile(filePath, posWorld, m_vertexBuf, m_indexBuf, m_vertexNormalBuf));
#else
  obj::LoadFromFile(filePath, posWorld, m_vertexBuf, m_indexBuf, m_vertexNormalBuf);
#endif
...
std::unique_ptr<cges::gameobject::PolygonalMesh> cges::MakePolygonalMesh(const RTCDevice device, 
                                                                         ...
                                                                         const ColorRGBA emissionColor,
                                                                         const material::Base& material) {
  return std::make_unique<gameobject::PolygonalMesh>(device, posWorld, filePath, diffuseColor, emissionColor, material);
}
